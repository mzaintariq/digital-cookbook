...
const checkboxClasses = computed(() => [
  ...
  props.disabled
    ? 'cursor-not-allowed opacity-50 bg-paper-100 dark:bg-paper-800 border-paper-400 dark:border-paper-700'
    : 'cursor-pointer',
  isIndeterminate.value || props.checked
    ? 'bg-brand-primary border-brand-primary text-paper-50'
    : 'bg-transparent border-paper-400 dark:border-paper-700 text-transparent hover:border-brand-primary',
].filter(Boolean).join(' '))
