<template>
  <div class="flex items-start gap-2 p-3 border border-paper-300 rounded-lg bg-paper-50 min-w-0 overflow-hidden">
    ...
    <div class="flex-1 min-w-0 space-y-2">
      <div class="flex items-start gap-2 min-w-0">
        ...
        <textarea v-model="step.description" rows="2" placeholder="eg: Preheat your oven to 375°F (190°C)..."
          class="flex-1 min-w-0 px-3 py-2 text-sm border border-paper-400 rounded focus:outline-none focus:ring-1 focus:ring-brand-primary" />
      ...
      <div v-if="step.subSteps && step.subSteps.length > 0" class="ml-6 space-y-2 min-w-0 overflow-hidden">
        <draggable v-model="step.subSteps" item-key="id" handle=".drag-handle" :animation="200" :force-fallback="true"
          ...
          <template #item="{ element: subStep, index: subIndex }">
            <div class="flex items-start gap-2 p-2 border border-paper-300 rounded bg-paper-50 min-w-0 overflow-hidden">
              ...
              <textarea v-model="subStep.description" rows="1" placeholder="Sub-step description..."
                class="flex-1 min-w-0 px-2 py-1 text-xs border border-paper-400 rounded focus:outline-none focus:ring-1 focus:ring-brand-primary" />
